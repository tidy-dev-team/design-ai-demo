...
// Get text property names for a mapping (from component property definitions)
export function getTextPropsForMappingId(mappingId: string): string[] {
  const mapping = getMappingById(mappingId);
  if (!mapping) {
    return [];
  }

  const component = getComponentByKey(mapping.componentKey);
  if (!component) {
    return [];
  }

  return component.props
    .filter((prop) => prop.figmaType === "TEXT")
    .map((prop) => prop.name);
}
